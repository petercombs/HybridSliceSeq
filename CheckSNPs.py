...
def class_refalt(snps_in_read, ref_alt_dict):
    snps_in_read = snps_in_read.dropna()
    snp_pos = snps_in_read.index
    snps_in_read = ''.join(snps_in_read)
    if snps_in_read == ''.join(ref_alt_dict[i-1][1] for i in snp_pos):
        return 1
    elif snps_in_read == ''.join(ref_alt_dict[i-1][0] for i in snp_pos):
        return -1
    return 0
